# LegalGovMCPRadar - 법률/정부 MCP 레이더

**🌐 Live Report**: https://ai-frendly-datahub.github.io/LegalGovMCPRadar/


Legal & Government 섹션의 한국 MCP 서버 목록을 수집하고 리스크/활성도를 추적하는 독립 Radar입니다.

## 프로젝트 목표

- **데이터 수집**: RSS 피드 및 API
- **엔티티 분석**: 도메인별 키워드 매칭
- **트렌드 리포트**: DuckDB 저장 + HTML 리포트로 법률/공공 MCP 동향 시각화
...
2. 실행:
   ```bash
   python main.py --category legal_gov_mcp --recent-days 7
   # 리포트: reports/legal_gov_mcp_report.html
   ```
...
- 워크플로: `.github/workflows/radar-crawler.yml`
  ...
  - 환경 변수 `RADAR_CATEGORY`는 `legal_gov_mcp`로 고정합니다.
...
- 설정 방법:
  ...
  3) 워크플로 파일의 `RADAR_CATEGORY`가 `legal_gov_mcp`인지 확인
...
- Source section: `Legal & Government`
- Primary motion: `compliance-risk`
- Governance: `high`
- Data quality priority: `P1`
...
- GitHub Pages visualization: `reports/index.html` (valid HTML); https://ai-frendly-datahub.github.io/LegalGovMCPRadar/.
