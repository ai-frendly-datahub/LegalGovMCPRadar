...
def test_korean_law_fake_stdio_fixture_collects_tool_result(monkeypatch) -> None:
    monkeypatch.setenv("KOREAN_LAW_API_KEY", "fixture-only")
    source = Source(
        ...
        config={
            "transport": "stdio",
            "command": sys.executable,
            "args": ["fixtures/mcp/fake_korean_law_mcp.py"],
            "tools": ["korean-law"],
            "env": ["KOREAN_LAW_API_KEY"],
            "timeout_seconds": 5,
            "max_items": 5,
        },
    )

    articles = collect_mcp_server_source(source, category="legal_gov_mcp", limit=5, timeout=5)

    assert len(articles) == 1
    assert articles[0].title == "Korean law fixture result"
    assert articles[0].link == "https://example.test/legal-gov/korean-law-fixture"
    assert articles[0].summary == "Fixture-only Korean law MCP result for collector normalization."
    assert articles[0].source == "chrisryugj/korean-law-mcp"
    assert articles[0].category == "legal_gov_mcp"


def test_finalchild_law_fake_stdio_fixture_collects_tool_results(monkeypatch) -> None:
    monkeypatch.setenv("LAW_API_OC", "fixture-only")
    source = Source(
        name="finalchild/law-mcp",
        type="mcp_server",
        url="https://github.com/finalchild/law-mcp",
        config={
            "transport": "stdio",
            "command": sys.executable,
            "args": ["fixtures/mcp/fake_finalchild_law_mcp.py"],
            "tools": ["search_laws", "get_law_details"],
            "env": ["LAW_API_OC"],
            "timeout_seconds": 5,
            "max_items": 5,
        },
    ...
    assert [article.title for article in articles] == [
        "Finalchild law search fixture",
        "Finalchild law details fixture",
    ]
    assert [article.link for article in articles] == [
        "https://example.test/legal-gov/finalchild-law-search",
        "https://example.test/legal-gov/finalchild-law-details",
    ]
    assert articles[0].summary == "Fixture-only law search result for collector normalization."
    assert articles[1].summary == "Fixture-only law detail result for collector normalization."
    assert {article.source for article in articles} == {"finalchild/law-mcp"}
    assert {article.category for article in articles} == {"legal_gov_mcp"}
